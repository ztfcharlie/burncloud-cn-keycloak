...
import { readManagedGitignoresFile, writeManagedGitignoreFiles } from "./managedGitignoreFiles";
...
export async function command(params) {
    ...
    await Promise.all(extensionModuleMetas
        ...
        if (ownedFilesRelativePaths.some(entry => same(entry, { isPublic, fileRelativePath }))) {
            return;
        }
        const destFilePath = pathJoin(isPublic
        ...
        skip_condition: {
            ...
            const destFileHash = computeHash(await fsPr.readFile(destFilePath));
