...
export async function command(params) {
    ...
    setup_src: {
        ...
        const doSetupAsStandalone = await (async () => {
            const relativeProjectDirPath = pathRelative(process.cwd(), buildContext.projectDirPath);
        ...
        if (doSetupAsStandalone) {
            const viteEnvDTsFilePath = pathJoin(buildContext.themeSrcDirPath, "vite-env.d.ts");
            ...
            await fs.copyFile(mainTsxFilePath, pathJoin(buildContext.themeSrcDirPath, "main-app.tsx"));
            files = [
                {
                    relativeFilePath: "main.tsx",
                    fileContent: [
    ...
    add_script: {
        const parsedPackageJson = await (async () => {
            ...
            const parsedPackageJson = JSON.parse((await fs.readFile(buildContext.packageJsonFilePath)).toString("utf8"));
        ...
        await fs.writeFile(buildContext.packageJsonFilePath, Buffer.from(packageJson_content, "utf8"));
    }
    const themeType = await (async () => {
        const values = [...THEME_TYPES, "email"].filter(themeType => {
    ...
    if (themeType === undefined) {
        console.log(chalk.gray("You already have implemented a theme type of every kind, exiting"));
        process.exit(0);
    }
