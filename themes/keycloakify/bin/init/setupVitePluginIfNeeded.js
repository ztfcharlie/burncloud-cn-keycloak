...
export async function setupVitePluginIfNeeded(params) {
    const { projectDirPath } = params;
    const viteConfigTsFilePath = pathJoin(projectDirPath, "vite.config.ts");
    if (!(await existsAsync(viteConfigTsFilePath))) {
        return;
    }
    const viteConfigTsContent = (await fs.readFile(viteConfigTsFilePath)).toString("utf8");
...
export default defineConfig({
    ...
    recast.visit(root, {
        visitProgram(path) {
            ...
            const importDeclaration = babelTypes.importDeclaration([
                babelTypes.importSpecifier(babelTypes.identifier("keycloakify"), babelTypes.identifier("keycloakify"))
        ...
        visitCallExpression(path) {
            ...
            babelTypes.isIdentifier(node.callee, { name: "defineConfig" }) &&
                ...
                if (pluginsProp && babelTypes.isArrayExpression(pluginsProp.value)) {
                    ...
                    const keycloakifyCall = babelTypes.callExpression(babelTypes.identifier("keycloakify"), [
                        babelTypes.objectExpression([
                            babelTypes.objectProperty(babelTypes.identifier("accountThemeImplementation"), babelTypes.stringLiteral("none"))
                        ])
                    ]);
    ...
    await fs.writeFile(viteConfigTsFilePath, Buffer.from(viteConfigTsContent_modified, "utf8"));
}
