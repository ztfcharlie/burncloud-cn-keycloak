...
export async function setupEslint(params) {
    ...
    const eslintConfigJsContent = (await fs.readFile(eslintConfigJsFilePath)).toString("utf8");
    ...
    recast.visit(root, {
        visitExportDefaultDeclaration(path) {
            ...
            if (!Array.isArray(args))
                return false;
            args.push(babelTypes.objectExpression([
                babelTypes.objectProperty(babelTypes.identifier("linterOptions"), babelTypes.objectExpression([
                    babelTypes.objectProperty(babelTypes.identifier("reportUnusedDisableDirectives"), babelTypes.stringLiteral("off"))
                ]))
            ]));
            return false;
        }
    ...
    await fs.writeFile(eslintConfigJsFilePath, Buffer.from(eslintConfigJsContent_modified, "utf8"));
}
