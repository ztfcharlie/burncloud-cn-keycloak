...
export async function appBuild(params) {
    ...
    if (entry === undefined) {
        console.log(chalk.red("There's multiple candidate script for building your app, name one 'build-keycloak-theme'"));
        process.exit(-1);
    }
    ...
    if (appBuildSubCommands.length === 0) {
        console.log(chalk.red(`Your ${scriptName} script should look like "... && keycloakify build ..."`));
        process.exit(-1);
    }
    common_case: {
    ...
    for (const subCommand of appBuildSubCommands) {
        ...
        const child = child_process.spawn(command, args, {
            ...
            env: Object.assign(Object.assign({}, process.env), { PATH: (() => {
                    ...
                    return [
                        pathJoin(pathDirname(buildContext.packageJsonFilePath), "node_modules", ".bin"),
