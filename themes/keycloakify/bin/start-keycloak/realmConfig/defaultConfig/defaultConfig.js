...
function getDefaultRealmJsonFilePath(params) {
    ...
    return pathJoin(getThisCodebaseRootDirPath(), "src", "bin", "start-keycloak", "realmConfig", "defaultConfig", `realm-kc-${keycloakMajorVersionNumber}.json`);
}
export const { getSupportedKeycloakMajorVersions } = (() => {
    ...
    function getSupportedKeycloakMajorVersions() {
        ...
        cache = fs
            .readdirSync(pathDirname(getDefaultRealmJsonFilePath({ keycloakMajorVersionNumber: 0 })))
            .map(fileBasename => {
...
export function getDefaultConfig(params) {
    const { keycloakMajorVersionNumber } = params;
    assert(getSupportedKeycloakMajorVersions().includes(keycloakMajorVersionNumber), `We do not have a default config for Keycloak ${keycloakMajorVersionNumber}`);
