...
import { CONTAINER_NAME, KEYCLOAKIFY_SPA_DEV_SERVER_PORT, KEYCLOAKIFY_LOGIN_JAR_BASENAME, TEST_APP_URL } from "../shared/constants";
import { SemVer } from "../tools/SemVer";
import { assert } from "tsafe/assert";
import * as fs from "fs";
import { join as pathJoin, relative as pathRelative, sep as pathSep, basename as pathBasename } from "path";
...
export async function command(params) {
    ...
    const { dockerImageTag } = await (async () => {
        ...
        console.log([
            ...
            chalk.gray("You can also explicitly provide the version with `npx keycloakify start-keycloak --keycloak-version 26` (or any other version)")
        ].join("\n"));
            ...
            while (true) {
                ...
                tag = value;
                {
                    ...
                    if (doImplementAccountMpa && tag.startsWith("22.")) {
                        console.log(chalk.yellow(`You are implementing a Multi-Page Account theme. Keycloak 22 is not supported, select another version`));
                ...
                };
                {
                    ...
                    if (doImplementAdminTheme && !getIsSupported(major)) {
                        console.log(chalk.yellow(`You are implementing an Admin theme. Only Keycloak 23 and later are supported, select another version`));
                ...
                }
                {
                    ...
                    if (doImplementAccountSpa && !getIsSupported(major)) {
                        console.log(chalk.yellow(`You are implementing a Single-Page Account theme. Only Keycloak 19 and later are supported, select another version`));
                        latestMajorTags_copy = latestMajorTags_copy.filter(tag => getIsSupported(readMajor(tag)));
                        continue;
                    }
                }
    ...
    const keycloakMajorVersionNumber = (() => {
        ...
        if (wrap === undefined) {
            try {
                ...
                console.error(chalk.yellow(`Keycloak version ${version.major} is not supported, supported versions are ${getSupportedKeycloakMajorVersions().join(", ")}`));
                process.exit(1);
            }
            catch (_a) {
                // NOTE: Latest version
                const [n] = getSupportedKeycloakMajorVersions();
                console.warn(chalk.yellow(`Could not determine the major Keycloak version number from the docker image tag ${dockerImageTag}. Assuming ${n}`));
                return n;
            }
        }
    ...
    })();
    {
        ...
        if (!isAppBuildSuccess) {
            console.log(chalk.red(`App build failed, exiting. Try building your app (e.g 'npm run build') and see what's wrong.`));
            process.exit(1);
        }
        ...
        if (!isKeycloakifyBuildSuccess) {
            console.log(chalk.red(`Keycloakify build failed, exiting. Try running 'npx keycloakify build' and see what's wrong.`));
            process.exit(1);
        }
    }
    ...
    const extensionJarFilePaths = [
        ...(keycloakMajorVersionNumber <= 20
            ? (console.log(chalk.yellow("WARNING: With older version of keycloak your changes to the realm configuration are not persisted")),
                [])
            : [
                pathJoin(getThisCodebaseRootDirPath(), "src", "bin", "start-keycloak", KEYCLOAKIFY_LOGIN_JAR_BASENAME)
            ]),
        ...(await Promise.all(buildContext.startKeycloakOptions.extensionJars.map(async (extensionJar) => {
            switch (extensionJar.type) {
                case "path": {
                    assert(await existsAsync(extensionJar.path), `${extensionJar.path} does not exist`);
    ...
    let parsedKeycloakThemesJson = id(undefined);
        ...
        await extractArchive({
            ...
            onArchiveFile: async ({ relativeFilePathInArchive, writeFile, readFile }) => {
                ...
                if (isInside({ dirPath: "theme", filePath: relativeFilePathInArchive })) {
                    await writeFile({
                        filePath: pathJoin(buildContext.keycloakifyBuildDirPath, relativeFilePathInArchive)
                    });
                }
            }
        });
    ...
    const { clientName, onRealmConfigChange, realmJsonFilePath, realmName, username } = await getRealmConfig({
        ...
        realmJsonFilePath_userProvided: await (async () => {
            ...
            if (buildContext.startKeycloakOptions.realmJsonFilePath !== undefined) {
                assert(await existsAsync(buildContext.startKeycloakOptions.realmJsonFilePath), `${pathRelative(process.cwd(), buildContext.startKeycloakOptions.realmJsonFilePath)} does not exist`);
    ...
    const jarFilePath_cacheDir = pathJoin(buildContext.cacheDirPath, pathBasename(jarFilePath));
    ...
    const dockerRunArgs = [
        ...
        ...extensionJarFilePaths.map(jarFilePath => `-v${SPACE_PLACEHOLDER}"${jarFilePath}":/opt/keycloak/providers/${pathBasename(jarFilePath)}`),
        ...(keycloakMajorVersionNumber <= 20
        ...
        ...[
            ...
            ...(fs.existsSync(pathJoin(buildContext.keycloakifyBuildDirPath, "theme", "account-v1"))
                ? ["account-v1"]
                : [])
        ]
            .map(themeName => ({
            localDirPath: pathJoin(buildContext.keycloakifyBuildDirPath, "theme", themeName),
    ...
    const child = child_process.spawn("docker", ["run", ...dockerRunArgs.map(line => line.split(SPACE_PLACEHOLDER)).flat()], { shell: true });
    ...
    const srcDirPath = pathJoin(buildContext.projectDirPath, "src");
    {
        ...
        const handler = async (data) => {
            ...
            console.log([
                "",
                `The ftl files from ${chalk.bold(`.${pathSep}${pathRelative(process.cwd(), pathJoin(buildContext.keycloakifyBuildDirPath, "theme"))}`)} are mounted in the Keycloak container.`,
                "",
                `Keycloak Admin console: ${chalk.cyan.bold(`http://localhost:${port}${kcHttpRelativePath !== null && kcHttpRelativePath !== void 0 ? kcHttpRelativePath : ""}`)}`,
                ...
                "",
                `Watching for changes in ${chalk.bold(`.${pathSep}${pathRelative(process.cwd(), buildContext.projectDirPath)}`)}`
            ].join("\n"));
        };
    ...
    }
    {
        ...
        chokidar
            ...
            ignore_path_covered_by_hmr: {
                ...
                ignore_patternfly: {
                    if (!isInside({
                        dirPath: pathJoin(buildContext.themeSrcDirPath, "shared", "@patternfly"),
                        filePath
                    })) {
                ...
                ignore_keycloak_ui_shared: {
                    if (!isInside({
                        dirPath: pathJoin(buildContext.themeSrcDirPath, "shared", "keycloak-ui-shared"),
                        filePath
                    })) {
