...
export async function command(params) {
    ...
    if (themeType === "account" &&
        ...
        console.log(`${chalk.red("✗")} Sorry, there is no Storybook support for Single-Page Account themes.`);
        process.exit(0);
        return;
    }
    if (themeType === "admin") {
        console.log(`${chalk.red("✗")} Sorry, there is no Storybook support for the Admin UI.`);
        process.exit(0);
        return;
    }
    console.log(`→ ${themeType}`);
    console.log(chalk.cyan("Select the page you want to create a Storybook for:"));
    const { value: pageId } = await cliSelect({
        values: (() => {
            switch (themeType) {
                case "login":
    ...
    console.log(`→ ${pageId}`);
    const componentBasename = capitalize(kebabCaseToCamelCase(pageId)).replace(/ftl$/, "stories.tsx");
    ...
    console.log([
        `${chalk.green("✓")} ${chalk.bold(pathJoin(".", pathRelative(process.cwd(), targetFilePath)))} copy pasted from the Keycloakify source code into your project`,
