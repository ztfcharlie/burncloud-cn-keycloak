...
export async function npmInstall(params) {
    ...
    const packageManagerBinName = (() => {
        ...
        for (const packageManager of packageMangers) {
            if (fs.existsSync(pathJoin(packageJsonDirPath, packageManager.lockFileBasename)) ||
    ...
    try {
        ...
        console.log(chalk.yellow(`\`${packageManagerBinName} install\` failed, continuing anyway...`));
    }
}
async function runPackageManagerInstall(params) {
    const { packageManagerBinName, cwd } = params;
    const dCompleted = new Deferred();
    const child = child_process.spawn(packageManagerBinName, ["install", ...(packageManagerBinName !== "npm" ? [] : ["--force"])], {
        cwd,
        env: process.env,
        shell: true
    });
...
async function installWithoutBreakingLinks(params) {
    ...
    fs.writeFileSync(pathJoin(tmpProjectDirPath, "package.json"), JSON.stringify(parsedPackageJson, undefined, 4));
    const YARN_LOCK = "yarn.lock";
    fs.copyFileSync(pathJoin(packageJsonDirPath, YARN_LOCK), pathJoin(tmpProjectDirPath, YARN_LOCK));
    await runPackageManagerInstall({
            ...
            if (basename.startsWith("@")) {
                return fs
                    ...
                    return {
                        ...
                        moduleDirPath: pathJoin(nodeModulesDirPath, basename, subBasename)
                    };
                })
    ...
    fs.cpSync(pathJoin(tmpProjectDirPath, YARN_LOCK), pathJoin(packageJsonDirPath, YARN_LOCK));
    rmSync(tmpProjectDirPath, { recursive: true });
    for (const scriptName of objectKeys(isImplementedScriptByName)) {
