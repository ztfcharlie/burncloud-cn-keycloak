...
export function replaceImportsInJsCode_webpack(params) {
    const { jsCode, buildContext, systemType = nodePath.sep === "/" ? "posix" : "win32" } = params;
    ...
    const getReplaceArgs = (language) => [
        ...
        (...[, n, u, matchedFunction, eForFunction]) => {
            ...
            ${n}[(function(){
                ...
                return "${u}";
            })()] = ${isArrowFunction ? `${e} =>` : `function(${e}) { return `} "/${WELL_KNOWN_DIRECTORY_BASE_NAME.DIST}/${staticDir}${language}/"`
                .replace(/\s+/g, " ")
                .trim();
        }
    ];
    fixedJsCode = fixedJsCode
        ...
        .replace(new RegExp(`[a-zA-Z]+\\.[a-zA-Z]+\\+"${staticDir.replace(/\//g, "\\/")}`, "g"), `window.kcContext["x-keycloakify"].resourcesPath + "/${WELL_KNOWN_DIRECTORY_BASE_NAME.DIST}/${staticDir}`);
    return { fixedJsCode };
}
