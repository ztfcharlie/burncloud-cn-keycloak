...
export async function command(params) {
    ...
    console.log([
        ...
        chalk.green(`Building the keycloak theme in .${pathSep}${pathRelative(process.cwd(), buildContext.keycloakifyBuildDirPath)} ...`)
    ].join(" "));
    const startTime = Date.now();
    {
        ...
        fs.writeFileSync(pathJoin(buildContext.keycloakifyBuildDirPath, ".gitignore"), Buffer.from("*", "utf8"));
    }
    ...
    run_post_build_script: {
        ...
        child_process.execSync("npx vite", {
            ...
            env: Object.assign(Object.assign({}, process.env), { [VITE_PLUGIN_SUB_SCRIPTS_ENV_NAMES.RUN_POST_BUILD_SCRIPT]: JSON.stringify({
                    resourcesDirPath,
                    buildContext
                }) })
        });
    ...
    console.log(chalk.green(`✓ keycloak theme built in ${((Date.now() - startTime) / 1000).toFixed(2)}s`));
}
