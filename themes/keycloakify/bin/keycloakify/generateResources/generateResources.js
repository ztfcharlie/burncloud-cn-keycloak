...
import { join as pathJoin, relative as pathRelative, dirname as pathDirname, basename as pathBasename } from "path";
import { replaceImportsInJsCode } from "../replacers/replaceImportsInJsCode";
import { replaceImportsInCssCode } from "../replacers/replaceImportsInCssCode";
import { generateFtlFilesCodeFactory } from "../generateFtl";
import { LOGIN_THEME_PAGE_IDS, ACCOUNT_THEME_PAGE_IDS, WELL_KNOWN_DIRECTORY_BASE_NAME, THEME_TYPES, KEYCLOAKIFY_SPA_DEV_SERVER_PORT } from "../../shared/constants";
...
export async function generateResources(params) {
    ...
    for (const themeType of [...THEME_TYPES, "email"]) {
        ...
        apply_replacers_and_move_to_theme_resources: {
            ...
            const destDirPath = pathJoin(themeTypeDirPath, "resources", WELL_KNOWN_DIRECTORY_BASE_NAME.DIST);
            ...
            if (themeType !== "login" &&
                ...
                transformCodebase({
                    srcDirPath: pathJoin(getThemeTypeDirPath({
                        themeName,
                        themeType: "login"
                    }), "resources", WELL_KNOWN_DIRECTORY_BASE_NAME.DIST),
                    destDirPath
                });
                break apply_replacers_and_move_to_theme_resources;
            }
            for (const directoryBasename of [
                ...
                const dirPath = pathJoin(buildContext.projectBuildDirPath, directoryBasename);
        ...
        i18n_single_page_account_legacy: {
            ...
            assert(moduleMeta !== undefined, `@keycloakify/keycloak-account-ui is supposed to be installed`);
            {
            ...
            isLegacyAccountSpa = true;
            const messagesDirPath_dest = pathJoin(getThemeTypeDirPath({ themeName, themeType: "account" }), "messages");
            transformCodebase({
                srcDirPath: messageDirPath_defaults,
                destDirPath: messagesDirPath_dest
            });
            apply_theme_changes: {
                const messagesDirPath_theme = pathJoin(buildContext.themeSrcDirPath, "account", "messages");
                ...
                fs.readdirSync(messagesDirPath_theme).forEach(basename => {
                    ...
                    const messages_src = propertiesParser.parse(fs.readFileSync(filePath_src).toString("utf8"));
                    const messages_dest = propertiesParser.parse(fs.readFileSync(filePath_dest).toString("utf8"));
                    ...
                    fs.writeFileSync(filePath_dest, Buffer.from(editor.toString(), "utf8"));
                });
            }
            languageTags = fs
        ...
        i18n_for_spas_and_native: {
            ...
            const messagesDirPath_theme = pathJoin(buildContext.themeSrcDirPath, themeType, isNative ? "messages" : "i18n");
            if (!fs.existsSync(messagesDirPath_theme)) {
                break i18n_for_spas_and_native;
            }
            ...
            writeMessagePropertiesFilesByThemeType[themeType] = ({ messageDirPath, themeName }) => {
                ...
                Object.entries(propertiesByLang).forEach(([lang, { base, override, overrideByThemeName }]) => {
                    ...
                    if (override !== undefined) {
                        const overrideMessages = propertiesParser.parse(override.toString("utf8"));
                        Object.entries(overrideMessages).forEach(([key, value]) => (messages[key] = value));
                    }
                    if (themeName in overrideByThemeName) {
                        const overrideMessages = propertiesParser.parse(overrideByThemeName[themeName].toString("utf8"));
                        Object.entries(overrideMessages).forEach(([key, value]) => (messages[key] = value));
                    }
                    const editor = propertiesParser.createEditor();
                    Object.entries(messages).forEach(([key, value]) => {
                    ...
                    fs.writeFileSync(pathJoin(messageDirPath, `messages_${lang}.properties`), Buffer.from(editor.toString(), "utf8"));
                });
            };
        }
        // NOTE: Legacy
        keycloak_static_resources: {
            ...
            transformCodebase({
                srcDirPath: pathJoin(getThisCodebaseRootDirPath(), "res", "public", WELL_KNOWN_DIRECTORY_BASE_NAME.KEYCLOAKIFY_DEV_RESOURCES, themeType),
        ...
        extensions_static_resources: {
            ...
            const dirPath = pathJoin(buildContext.publicDirPath, KEYCLOAK_THEME, themeType);
            if (!fs.existsSync(dirPath)) {
                break extensions_static_resources;
            }
            transformCodebase({
                srcDirPath: dirPath,
                destDirPath: pathJoin(themeTypeDirPath, "resources", WELL_KNOWN_DIRECTORY_BASE_NAME.DIST, KEYCLOAK_THEME, themeType)
            });
        }
        bring_in_account_v1: {
            if (isNative) {
        ...
        generate_theme_properties: {
            const getEnvironmentVariableLines = (environmentVariables) => environmentVariables.map(({ name, default: defaultValue }) => `${name}=\${env.${name}:${escapeStringForPropertiesFile(defaultValue)}}`);
            if (isNative) {
                fs.writeFileSync(pathJoin(themeTypeDirPath, "theme.properties"), Buffer.from([
                    fs.readFileSync(pathJoin(themeTypeDirPath, "theme.properties")),
    ...
    for (const themeVariantName of [...buildContext.themeNames].reverse()) {
        for (const themeType of [...THEME_TYPES, "email"]) {
            ...
            run_writeMessagePropertiesFiles: {
                ...
                writeMessagePropertiesFiles({
                    messageDirPath: pathJoin(getThemeTypeDirPath({ themeName: themeVariantName, themeType }), "messages"),
                    themeName: themeVariantName
                });
            }
        }
