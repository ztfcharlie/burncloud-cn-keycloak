import { FALLBACK_LANGUAGE_TAG } from "../../shared/constants";
...
export function generateMessageProperties(params) {
    ...
    const baseMessagesDirPath = pathJoin(getThisCodebaseRootDirPath(), "src", themeType, "i18n", "messages_defaultSet");
    const messages_defaultSet_by_languageTag_defaultSet = Object.fromEntries(fs
        ...
        const messages = JSON5.parse(messagesJson);
    ...
    const { i18nTsFilePath } = (() => {
        ...
        files = files.filter(file => fs.readFileSync(file).toString("utf8").includes("i18nBuilder"));
    ...
    const messages_defaultSet_by_languageTag_notInDefaultSet = (() => {
        ...
        const messages_defaultSet_by_languageTag_notInDefaultSet = Object.fromEntries(Object.entries(extraLanguageEntryByLanguageTag).map(([languageTag, { path: relativePathWithoutExt }]) => [
            ...
            (() => {
                ...
                assert(declarationCode !== undefined, `${filePath} does not contain a 'messages' variable declaration`);
                let messages = {};
                try {
    ...
    const messages_themeDefined_by_languageTag = (() => {
        ...
        recast.visit(i18nTsRoot, {
            visitCallExpression: function (path) {
                ...
                if (node.callee.type === "MemberExpression" &&
                    ...
                    firstArgumentCode = babelGenerate(node.arguments[0]).code;
                    return false;
                }
                this.traverse(path);
            }
        ...
        try {
            eval(`${symToStr({ messages_themeDefined_by_languageTag })} = ${firstArgumentCode}`);
        }
        catch (_a) {
            console.warn([
        ...
        writeMessagePropertiesFiles: ({ messageDirPath, themeName }) => {
            for (const languageTag of languageTags) {
                ...
                const propertiesFileSource = [
                    "",
                    ...Object.entries(messages).map(([key, value]) => `${key}=${escapeStringForPropertiesFile(value)}`),
                    ""
                ].join("\n");
                fs.mkdirSync(messageDirPath, { recursive: true });
                fs.writeFileSync(pathJoin(messageDirPath, `messages_${languageTag.replace(/\-/g, "_")}.properties`), Buffer.from(propertiesFileSource, "utf8"));
            }
        }
    };
}
