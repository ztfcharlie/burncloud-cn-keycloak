...
import { readManagedGitignoresFile, writeManagedGitignoreFiles } from "./sync-extensions/managedGitignoreFiles";
...
export async function command(params) {
    ...
    const { targetFileRelativePathsByExtensionModuleMeta } = await (async () => {
        const fileOrDirectoryRelativePath = (() => {
            ...
            return pathRelative(dirPath_ctx, getAbsoluteAndInOsFormatPath({
                cwd: dirPath_ctx,
    ...
    if (targetFileRelativePathsByExtensionModuleMeta.size === 0) {
        console.log(chalk.yellow("There is no Keycloakify extension modules files matching the provided path."));
        process.exit(1);
    }
    const { ownedFilesRelativePaths: ownedFilesRelativePaths_current } = await readManagedGitignoresFile({
        buildContext
    });
...
async function command_own(params) {
    ...
    await writeManagedGitignoreFiles({
        ...
        ownedFilesRelativePaths: [
            ...
            ...Array.from(targetFileRelativePathsByExtensionModuleMeta.values())
                .flat()
                .filter(fileRelativePath => !ownedFilesRelativePaths_current.some(entry => same(entry, { isPublic, fileRelativePath })))
    ...
    for (const [extensionModuleMeta, fileRelativePaths] of targetFileRelativePathsByExtensionModuleMeta.entries()) {
        ...
        for (const fileRelativePath of fileRelativePaths) {
            if (ownedFilesRelativePaths_current.some(entry => same(entry, { isPublic, fileRelativePath }))) {
                console.log(chalk.grey(`You already have ownership over '${fileRelativePath}'.`));
                continue;
            }
            writeActions.push(async () => {
                ...
                await fsPr.writeFile(pathJoin(isPublic
                    ? pathJoin(buildContext.publicDirPath, KEYCLOAK_THEME)
...
async function command_revert(params) {
    ...
    const ownedFilesRelativePaths_toRemove = Array.from(targetFileRelativePathsByExtensionModuleMeta.values())
        ...
        if (!ownedFilesRelativePaths_current.some(entry => same(entry, { isPublic, fileRelativePath }))) {
            console.log(chalk.grey(`Ownership over '${fileRelativePath}' wasn't claimed.`));
            return false;
        }
        console.log(chalk.green(`Ownership over '${fileRelativePath}' relinquished.`));
        return true;
    });
    ...
    await writeManagedGitignoreFiles({
        buildContext,
        extensionModuleMetas,
        ownedFilesRelativePaths: ownedFilesRelativePaths_current.filter(entry => !ownedFilesRelativePaths_toRemove.some(fileRelativePath => same(entry, { isPublic, fileRelativePath })))
    });
